...
def get_conn():
    conn=MySQLdb.connect(db="EXETOOLS_DB",host="localhost",user="root",passwd="root", use_unicode=True,charset="utf8")
    cursor=conn.cursor()
    return conn,cursor
...
class Data():
    ...
    file_name = "TlS_Posts_EXE_%s.csv"%(str(datetime.datetime.now().date()))
    oupf = open(os.path.join(csv_path, file_name), "wb+")
    todays_excel_file = csv.writer(oupf)
    headers = ['Domain','crawl_type', 'Category','SubCategory', 'Thread Title', 'Thread Url', 'Post id', 'Post` Url', 'Publish Time', 'Fetch Time', 'author', "author_url", 'Text', ' all_links','reference_url']
    todays_excel_file.writerow(headers)
    excel_auth_name1 = "TLS_Author_EXE_%s.csv"%(str(datetime.datetime.now().date()))
    ...
    query = 'SELECT * from exe_posts'
    cursor.execute(query)
    row = cursor.fetchall()
    for i in row:
        todays_excel_file.writerow(i)
    que = 'SELECT * from exe_authors'
    cursor.execute(que)
